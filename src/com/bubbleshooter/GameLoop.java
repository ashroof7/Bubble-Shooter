...
public class GameLoop extends Thread {
	...
	void updateGame() {
		if (gamePanel.isfired) {
			...
			int row = (MainGame.drawOffset - gamePanel.bulletLoc.y)
					/ MainGame.DIAM;
			...
			if (newX + MainGame.DIAM > gamePanel.displayDims.x || newX < 0
				...
				// System.out.println("A "+gamePanel.map.length
				// +" "+gamePanel.map[0].length);
				// System.out.println("Aaaa "+newRow +" "+newCol);
				if (newRow < gamePanel.map.length
						&& gamePanel.map[newRow][newCol] != -1) {
